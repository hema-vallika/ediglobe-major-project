...
import AddFeeForm from "./form/Add-fee-form"
...
export default function FeesPage() {
  ...
  const filteredRecords = feesRecords.filter((record) => {
    const matchesSearch =
  ...
  // Calculate totals
  const totalRevenue = feesRecords.reduce((sum, record) => sum + record.paidAmount, 0)
  const totalPending = feesRecords.reduce((sum, record) => sum + record.pendingAmount, 0)
  const paidCount = feesRecords.filter((record) => record.status === "Paid").length
  const overdueCount = feesRecords.filter((record) => record.status === "Overdue").length
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <section className="bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div>
              <h1 className="text-3xl md:text-4xl font-bold mb-2">Fees Management</h1>
              <p className="text-slate-300 text-lg">Track student fees, payments, and financial records</p>
            </div>
      ...
      <section className="py-6">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white rounded-lg shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200">
              <h2 className="text-lg font-semibold text-slate-800">Fee Records ({filteredRecords.length} found)</h2>
            </div>
                    ...
                    <tr key={record.id} className="hover:bg-slate-50">
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div>
                          ...
                          <div className="text-sm text-slate-500">{record.department}</div>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-slate-900">{record.id}</div>
                        <div className="text-sm text-slate-500">{record.semester}</div>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        {record.transactionId && (
                          <div className="text-xs text-slate-400">ID: {record.transactionId}</div>
                        )}
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
            ...
            {filteredRecords.length === 0 && (
              <div className="text-center py-12">
                ...
                <h3 className="text-lg font-medium text-slate-900 mb-2">No fee records found</h3>
        ...
        <AddFeeForm isOpen={isAddFormOpen} onClose={() => setIsAddFormOpen(false)} onSubmit={handleAddFee} />
      <Footer />
    </div>
  )
}
